	...
	public function copylotdata(){
      ...
      //dd($lot_no);
			if(empty($lot_no))
			{
				...
				$lot_array = array(
					'lot_title'=>$lotData->make.' '.$lotData->model.' '.$lotData->trim,
			...
			for($year=$yearMax; $year>=$yearMin; $year--){
				foreach ($vehicleTypes as  $vehicleType) {
					for($page=1; $page<=40; $page++){
						...
						// $url='https://carsfromwest.com/api/auction/lots?auctions=iaai&type=AM&yearMin=2022&yearMax=2022&onlyActive=true&page=1';
					// $curl = curl_init();
					// curl_setopt($curl, CURLOPT_URL, $url);
					// curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
					// curl_setopt($curl, CURLOPT_HEADER, false);
    ...
    // dd($lot_data->data[0]);
      ...
      //dd($lot_no);
								...
								if(isset($lotinfo->odometer->unit)){
									...
									$odometer=$lotinfo->odometer->value;
									$mileage=$lotinfo->odometer->value.' '.$lotinfo->odometer->unit;
									if(isset($lotinfo->odometer->status) && $lotinfo->odometer->status==1){
										$mileage.='(actual)';
									}else{
										$mileage.='(not actual)';
									}
     					      ...
     					      // dd($data1);
						        $trim = isset($lotData->trim) ? $lotData->trim : '';
								$lot_array = array(
									'lot_title'=>$lotData->make.' '.$lotData->model.' '. $trim,
									'title_year_before'=>$madeyear.' '.$lotData->make.' '.$lotData->model.' '.$trim,
									'title_year_after'=>$lotData->make.' '.$lotData->model.' '.$trim.' '.$madeyear,
									'feature_image'=>$feature_image,
									'lot_no'=>isset($value->attributes->auctionLotId)? $value->attributes->auctionLotId:'',
									'trading_date'=>isset($value->attributes->auctionDate) ? $value->attributes->auctionDate:'' ,
									'vin'=>isset($value->attributes->lotData->vin)? $value->attributes->lotData->vin:'',
